...
def reading_items() -> list[dict]:
    ...
    out = []
    for f in sorted(STRUCT.rglob("*.json")):
        if "reading" not in f.parts:
            continue
        try:
    ...
    # 按 id 去重：同一题目 id 可能被多个数据包加载（如格式示范包被重打包）。
    # items 表以 id 为主键（INSERT OR REPLACE），search 表是 FTS5 无主键，
    # 若不去重会导致 index.jsonl / items / search 三处数量不一致，触发契约点 C。
    # 去重保留最后一个（后加载的包覆盖先加载的）。
    seen: dict[str, dict] = {}
    for it in out:
        seen[it["id"]] = it
...
def speaking_items() -> list[dict]:
    ...
    out = []
    for f in sorted(STRUCT.rglob("*.jsonl")):
        if "speaking" not in f.parts:
            continue
    ...
    # 与 reading_items 相同的按 id 去重逻辑，保持契约点 C 三处数量一致
    seen: dict[str, dict] = {}
    for it in out:
        seen[it["id"]] = it
    return list(seen.values())
...
def main() -> int:
    ...
    try:
        print(f"\n→ {INDEX.relative_to(repo_root())}")
        print(f"→ {DB.relative_to(repo_root())} (items + search FTS5)")
    except ValueError:
        # 数据目录在仓库外（ELK_DATA_DIR 指向外部）时，相对 repo_root 无效，直接打印绝对路径
        print(f"\n→ {INDEX}")
        print(f"→ {DB} (items + search FTS5)")
    return 0
